...
export class WeekComponent implements OnInit {
  ...
  workLogsDay: Worklog2[] = [];
  weekDates: WeekDate[] = [];
  constructor(private dateService: DateService, private router: Router, private worklogService: Worklog2Service, private authService : AuthService) { }
      ...
      this.authService.token.subscribe(x => { if(x)
        if(x.access_token != null){
          this.worklogService.getWorkedHoursDay(this.monthSelected.getFullYear(), this.monthSelected.getMonth(),
              i).subscribe( x => {
            this.counter++;
            this.workLogsDay = x as Worklog2[];
            if(this.workLogsDay.length > 0){
              ...
              let date = new Date(this.workLogsDay[0].date);
              console.log(date);
              ...
              this.weekDates[this.counter-1].weekDate = this.weekNames[date.getDay()] + ". " + date.getDate() + " " + this.monthNames[date.getMonth()];
            }else{
              let date2 = new Date(this.monthSelected.getFullYear(), this.monthSelected.getMonth(), i);
              this.weekDates[this.counter-1] = new WeekDate(new Hours(0,0,0), this.weekNames[date2.getDay()]  + ". " +  i + " " + this.monthNames[this.monthSelected.getMonth()], date2);
            }
